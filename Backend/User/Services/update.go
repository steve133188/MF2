...
func UpdateDivisionAndTeam(c *fiber.Ctx) error {
	...
	update := bson.M{"$set": bson.M{"division_name": c.Params("div"), "team_name": c.Params("team")}}

	err := usersCollection.FindOneAndUpdate(c.Context(), bson.D{{Key: "username", Value: c.Params("name")}}, update).Decode(&user)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"success": false,
